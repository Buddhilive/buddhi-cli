...
1. **Update code graph**: Run `buddhi generate` via the `terminal-runner` agent to ensure
   ...
   investigation.
2. Gather the symptom from `$ARGUMENTS`. If any of the following is missing,
...
3. Before reading raw source, consult `.buddhi/docs/` and use Buddhi MCP tools
...
4. Form a ranked list of candidate root causes, most likely first, based on
   what step 3 turned up. If the failing area spans domains ambiguously
...
5. Investigate systematically rather than jumping at the first plausible
   ...
   that evidence.
6. Once the root cause is confirmed, report it with file:line references
