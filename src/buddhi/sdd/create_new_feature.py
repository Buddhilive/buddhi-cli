...
def _get_highest_from_specs(specs_dir: Path) -> int:
    ...
    for entry in specs_dir.iterdir():
        ...
        if re.match(r"^[0-9]{3,}-", name) and not re.match(
            ...
            if m is not None:
                number = _int64_from_digits(m.group())
...
def main(argv: list[str] | None = None) -> int:
    ...
    specs_dir = repo_root / ".buddhi" / "specs"
    if not args.dry_run:
    ...
    if args.use_timestamp:
        feature_num = datetime.datetime.now(datetime.timezone.utc).strftime("%Y%m%d-%H%M%S")
    else:
        if branch_number:
    ...
    feature_assignment, directory_assignment = "", ""
    if not args.dry_run:
        ...
        persist_feature_json(repo_root, f".buddhi/specs/{branch_name}")
